# ───────────────────────────────────────────────────────────────────
# AutoTrader scraper, Playwright for JS-rendered car search
# ───────────────────────────────────────────────────────────────────
# AutoTrader is a large car marketplace. Uses Playwright because
# the site is heavily JS-rendered.
# ───────────────────────────────────────────────────────────────────
...
class AutoTraderScraper(BaseScraper):
    """Scrapes AutoTrader for car listings in Arizona."""

    def __init__(self, config: Config):
        super().__init__(config)
        self.source_name = "autotrader"
    ...
    def _build_search_url(self) -> str:
        ...
        # AutoTrader search URL with Arizona zip, price sort ascending
        return (
            f"https://www.autotrader.com/cars-for-sale/all-cars/{query}"
            f"?zip=85001"
            f"&searchRadius=50"
    ...
    def _fetch_page(self, url: str) -> str:
        """Fetch AutoTrader page using Playwright."""
        try:
    ...
    def _parse_listing(self, card: BeautifulSoup) -> Optional[ScrapedListing]:
        """Parse an AutoTrader listing card."""
        # Title
        title_el = card.select_one("[class*='title' i], h2, h3, a[data-cmp='inventoryListing_title']")
        if not title_el:
        ...
        # Price
        price_el = card.select_one("[class*='price' i], [data-cmp='firstPrice']")
        if not price_el:
            return None
        ...
        # URL
        link_el = card.select_one("a[href*='/vehicledetails/'], a[href*='/cars-for-sale/']")
        if not link_el:
        ...
        return ScrapedListing(
            ...
            listing_id=f"autotrader-{listing_id}",
            title=title,
            price_usd=price,
            url=url,
            condition="Used",
    ...
    def scrape(self) -> list[ScrapedListing]:
        ...
        try:
            ...
            cards = (
                soup.select("[class*='inventory' i]")
                or soup.select("[class*='listing' i]")
                or soup.select("[data-cmp='inventoryListing']")
        ...
        print(f"  [AutoTrader] Found {len(found)} matching listings")
        return found
